...
const MODEL_INFO: Record<string, TableInfo> = {
  ...
  customer: {
    ...
    relations: {
      ...
      addresses: 'address',
      orders: 'order',
      defaultShipMethod: 'shippingMethod',
    },
  },
  supplier: {
    name: 'supplier',
...
export function parseWhereClause(where: string | undefined, tableInfo: TableInfo): any {
  ...
  if (inMatch) {
    ...
    if (!tableInfo.fields.includes(field)) {
      throw new Error(
        `Field '${field}' not found in table '${tableInfo.name}'. Available fields: ${tableInfo.fields.join(', ')}`
      );
    }
  ...
  const dateMatch = where.match(
    /^(\w+)\s*([<>]=?)\s*(NOW\(\)|CURRENT_DATE|CURRENT_TIMESTAMP)(?:\s*([+-])\s*INTERVAL\s*'([^']+)')?$/i
  );
  if (dateMatch) {
  ...
  if (where.match(/\b(AND|OR)\b/i)) {
    ...
    throw new Error(
      `Complex WHERE clauses with mixed AND/OR not yet supported. Use simple conditions or multiple queries.`
    );
  }
  ...
  if (fieldCompMatch) {
    ...
    if (sourceTable === tableInfo.name && targetTable === tableInfo.name) {
      if (!tableInfo.fields.includes(field1)) {
        throw new Error(
          `Field '${field1}' not found in table '${tableInfo.name}'. Available fields: ${tableInfo.fields.join(', ')}`
        );
      }
      if (!tableInfo.fields.includes(field2)) {
        throw new Error(
          `Field '${field2}' not found in table '${tableInfo.name}'. Available fields: ${tableInfo.fields.join(', ')}`
        );
      }
    }
  ...
  throw new Error(
    `WHERE clause syntax not recognized: "${where}". Use simple conditions like "field = 'value'", "field = true", "field > 10", or field comparisons like "field1 <= field2"`
  );
}
...
export async function executeCount(
  ...
  if (
    where &&
    (where._fieldComparison ||
      where._inClause ||
      where._likeClause ||
      where._nullCheck ||
      where._dateComparison ||
      where._andClause)
  ) {
    ...
    if (where._fieldComparison) {
      ...
      if (sourceTable === targetTable) {
        ...
        if (!joinInfo) {
          throw new Error(
            `Cannot determine relationship between tables '${tableName}' and '${targetTable}'`
          );
        }
      ...
      where._andClause.forEach((clause: any) => {
        if (clause._fieldComparison) {
          ...
          if (
            ...
            const enumType = getEnumTypeName(tableName, field);
            clauses.push(
              `"${toSnakeCase(field)}" IN (${values.map((_, i) => `$${paramIndex + i}::"${enumType}"`).join(', ')})`
            );
          } else {
...
function getJoinInfo(
  fromTable: string,
  toTable: string
): { joinClause: string; tables: string[] } | null {
