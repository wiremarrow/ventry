        ...
        <div className="flex flex-col gap-6">
            ...
            <Card className="p-6">
              <div className="space-y-2">
                ...
                <p className="text-2xl font-bold">
                  {customers?.customers?.filter((c) => (c.metrics?.orderCount || 0) > 0).length ||
                    0}
                </p>
              </div>
            </Card>
                ...
                <p className="text-2xl font-bold">
                  {formatCurrency(
                    customers?.customers?.reduce(
                      (sum, c) => sum + (c.metrics?.lifetimeRevenue || 0),
                      0
                    ) || 0
                  )}
                </p>
                  ...
                  {formatCurrency(
                    customers?.customers && customers.customers.length > 0
                      ? customers.customers.reduce(
                          (sum, c) =>
                            sum +
                            (typeof c.metrics?.avgOrderValue === 'number'
                              ? c.metrics.avgOrderValue
                              : parseFloat(c.metrics?.avgOrderValue?.toString() || '0')),
                          0
                ...
                {isLoading ? (
                  ...
                  customers?.customers?.map((customer) => (
                    <TableRow key={customer.id}>
                        ...
                        {customer._count?.addresses > 0 && (
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm">
                              {customer._count.addresses} address
                      ...
                      <TableCell>
                        <Badge variant="secondary">{customer.metrics.orderCount} orders</Badge>
                      </TableCell>
                      <TableCell>{formatCurrency(customer.metrics.lifetimeRevenue || 0)}</TableCell>
                      <TableCell>
                        <DropdownMenu>
          ...
          <Dialog open={!!editCustomer} onOpenChange={() => setEditCustomer(null)}>
            <DialogContent className="max-w-2xl">
              ...
              {editCustomer && (
                <CustomerForm customer={editCustomer} onSuccess={handleEditSuccess} />
              )}
            </DialogContent>
          </Dialog>
        </div>
