...
export function PurchaseOrderStats() {
  ...
  useEffect(() => {
    if (currentMonthData && prevMonthData && pendingData && overdueData) {
      const currentTotal = currentMonthData.purchaseOrders.reduce((sum, po) => sum + po.total, 0);
      const prevTotal = prevMonthData.purchaseOrders.reduce((sum, po) => sum + po.total, 0);

      const orderChange =
        prevMonthData.pagination.total > 0
          ? ((currentMonthData.pagination.total - prevMonthData.pagination.total) /
              prevMonthData.pagination.total) *
            100
          : 0;

      const valueChange = prevTotal > 0 ? ((currentTotal - prevTotal) / prevTotal) * 100 : 0;
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Total Value (This Month)</CardTitle>
