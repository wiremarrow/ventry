...
export async function createDirectCaller(
  contextOverride?: Partial<Awaited<ReturnType<typeof createContext>>>
) {
...
export async function createAuthenticatedContext(user: {
  id: string;
  email: string;
  role: string;
}) {
