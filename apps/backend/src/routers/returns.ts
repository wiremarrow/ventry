...
export const returnsRouter = createTRPCRouter({
  ...
  list: organizationProcedure.input(returnFilterSchema).query(async ({ ctx, input }) => {
    const {
      search,
      // type, // Not in schema
      status,
  ...
  // Create a new return
  create: organizationProcedure.input(returnCreateSchema).mutation(async ({ ctx, input }) => {
    const { customerId, orderId, reason, refundAmount, restockFee, rmaNumber, notes, items } =
      input;
  ...
  // Update return
  update: organizationProcedure.input(returnUpdateSchema).mutation(async ({ ctx, input }) => {
  ...
  approve: organizationProcedure.input(returnApprovalSchema).mutation(async ({ ctx, input }) => {
  ...
  ship: organizationProcedure.input(returnShipmentSchema).mutation(async ({ ctx, input }) => {
    const { id, carrier, trackingNumber, shippedDate, estimatedDelivery, shippingCost, notes } =
      input;
  ...
  receive: organizationProcedure.input(returnReceiveSchema).mutation(async ({ ctx, input }) => {
    ...
    const result = await ctx.prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (
          item.dispositionAction === 'RETURN_TO_STOCK' &&
          item.locationId &&
          item.qtyReceived > 0
        ) {
    ...
    .query(async ({ ctx, input }) => {
      ...
      const processingTimes = completedReturns.map((r) => {
        const days = Math.ceil(
          (r.returnDate.getTime() - r.createdAt.getTime()) / (1000 * 60 * 60 * 24)
        );
        return days;
      });
  ...
  exportReturns: organizationProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const ret of returns) {
        ...
        const customerSupplier = ret.order?.customer
