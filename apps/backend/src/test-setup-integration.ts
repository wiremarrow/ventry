...
function setupWorkerDatabase() {
  ...
  if (process.env.DATABASE_URL_BASE && workerId) {
    ...
    console.log(
      `🚀 Integration Tests Worker ${workerId}: Using database:`,
      workerDbUrl.replace(/\/\/.*@/, '//***@')
    );
    ...
    console.log(
      '🚀 Integration Tests: Using CI database (single worker):',
      singleDbUrl.replace(/\/\/.*@/, '//***@')
    );
  } else if (process.env.DATABASE_URL) {
    // Single database provided (environment override)
    console.log(
      '🚀 Integration Tests: Using environment-provided database:',
